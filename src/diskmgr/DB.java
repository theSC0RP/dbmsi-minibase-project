...
public class DB implements GlobalConst {
  ...
  public void read_page(PageId pageno, Page apage)
    ...
    if (
      (pageno.pid < 0) || (pageno.pid >= num_pages)
    ...
    try {
      fp.read(buffer);
      ReadWriteCounter.incrementReadCount();
    } catch (IOException e) {
  ...
  public void write_page(PageId pageno, Page apage)
    ...
    try {
      ...
      ReadWriteCounter.incrementWriteCount();
    } catch (IOException e) {
      throw new FileIOException(e, "DB file I/O error");
    }
  }
  ...
  public void allocate_page(PageId start_page_num)
    throws OutOfSpaceException, InvalidRunSizeException, InvalidPageNumberException, FileIOException, DiskMgrException, IOException {
  ...
  public void allocate_page(PageId start_page_num, int runsize)
    throws OutOfSpaceException, InvalidRunSizeException, InvalidPageNumberException, FileIOException, DiskMgrException, IOException {
    if (runsize < 0) throw new InvalidRunSizeException(
      null,
      "Negative run_size"
    );
    ...
    for (int i = 0; i < num_map_pages; ++i) { // start forloop01
      ...
      if (num_bits_this_page > bits_per_page) num_bits_this_page =
        bits_per_page;
      ...
      for (
        ;
        num_bits_this_page > 0 && current_run_length < run_size;
        ++byteptr
      ) { // start forloop02
  ...
  public void deallocate_page(PageId start_page_num, int run_size)
    ...
    if (run_size < 0) throw new InvalidRunSizeException(
      null,
      "Negative run_size"
    );
  ...
  public void add_file_entry(String fname, PageId start_page_num)
    throws FileNameTooLongException, InvalidPageNumberException, InvalidRunSizeException, DuplicateEntryException, OutOfSpaceException, FileIOException, IOException, DiskMgrException {
    if (fname.length() >= MAX_NAME) throw new FileNameTooLongException(
      null,
      "DB filename too long"
    );
    if (
      (start_page_num.pid < 0) || (start_page_num.pid >= num_pages)
    ) throw new InvalidPageNumberException(null, " DB bad page number");

    // Does the file already exist?

    if (get_file_entry(fname) != null) throw new DuplicateEntryException(
      null,
  ...
  public void delete_file_entry(String fname)
    ...
    do { // startDO01
      ...
      while (entry < dp.getNumOfEntries()) {
        ...
        if (
          (tmppid.pid != INVALID_PAGE) && (tmpname.compareTo(fname) == 0)
        ) break;
        entry++;
      }
    ...
    if (
      !found
    ) throw new FileEntryNotFoundException(null, "DB file not found"); // Entry not found - nothing deleted
  ...
  public PageId get_file_entry(String name)
    ...
    do { // Start DO01
      ...
      while (entry < dp.getNumOfEntries()) {
        ...
        if (
          (tmppid.pid != INVALID_PAGE) && (tmpname.compareTo(name) == 0)
        ) break;
        entry++;
      }
      if (entry < dp.getNumOfEntries()) {
    ...
    if (
      !found
    ) { // Entry not found - don't post error, just fail.
    ...
    for (int i = 0; i < num_map_pages; i++) { //start forloop01
      ...
      for (; num_bits_this_page > 0; pgptr++) { // start forloop02
        for (
          ...
          if ((bit_number % 10) == 0) if ((bit_number % 50) == 0) {
            if (bit_number > 0) System.out.println("\n");
            System.out.print("\t" + bit_number + ": ");
          } else System.out.print(' ');

          if (bit != 0) System.out.print("1"); else System.out.print("0");
        } //end of forloop03
      } //end of forloop02
  ...
  private void set_bits(PageId start_page, int run_size, int bit)
    ...
    if (
      (start_page.pid < 0) || (start_page.pid + run_size > num_pages)
    ...
    for (
      PageId pgid = new PageId(first_map_page);
      ...
      if (last_bit_no >= bits_per_page) last_bit_no = bits_per_page - 1;
      ...
      for (; cur_posi <= last_byte_no; ++cur_posi, first_bit_offset = 0) { //start forloop02
        ...
        int num_bits_this_byte =
          (run_size > max_bits_this_byte ? max_bits_this_byte : run_size);
...
class DBHeaderPage implements PageUsedBytes, GlobalConst {
  ...
  public DBHeaderPage(Page page, int pageusedbytes) throws IOException {
    ...
    for (int index = 0; index < num_entries; ++index) initFileEntry(
      INVALID_PAGE,
      index
    );
  }
