...
class BufHashTbl implements GlobalConst {
  ...
  public int lookup(PageId pageNo) {
    BufHTEntry ent;
    if (pageNo.pid == INVALID_PAGE) return INVALID_PAGE;
  ...
  public boolean remove(PageId pageNo) {
    ...
    if (pageNo.pid == INVALID_PAGE) return true;

    int indx = hash(pageNo);
    for (cur = ht[indx]; cur != null; cur = cur.next) {
      if (cur.pageNo.pid == pageNo.pid) break;
      prev = cur;
    }

    if (cur != null) {
      if (prev != null) prev.next = cur.next; else ht[indx] = cur.next;
    } else {
      System.err.println(
        "ERROR: Page " + pageNo.pid + " was not found in hashtable.\n"
      );
...
class Clock extends Replacer {
  ...
  public int pick_victim()
    ...
    while (state_bit[head].state != Available) {
      if (state_bit[head].state == Referenced) state_bit[head].state =
        Available;
...
public class BufMgr implements GlobalConst {
  ...
  private void privFlushPages(PageId pageid, int all_pages)
    ...
    for (
      ...
      i < numBuffers;
      i++
    ) if ((all_pages != 0) || (frmeTable[i].pageNo.pid == pageid.pid)) { // write all valid dirty pages to disk
      if (frmeTable[i].pin_count() != 0) unpinned++;
      ...
      if (all_pages == 0) {
        if (unpinned != 0) throw new PagePinnedException(
          null,
          "BUFMGR: PAGE_PINNED."
        );
      }
  ...
  public BufMgr(int numbufs, String replacerArg) {
    ...
    for (
      ...
      i < numBuffers;
      i++
    ) frmeTable[i] = new FrameDesc(); // initialize frameTable
  ...
  public void pinPage(PageId pin_pgid, Page page, boolean emptyPage)
    throws ReplacerException, HashOperationException, PageUnpinnedException, InvalidFrameNumberException, PageNotReadException, BufferPoolExceededException, PagePinnedException, BufMgrException, IOException {
    int frameNo;
    boolean bst, bst2;
    ...
    if (frameNo < 0) { // Not in the buffer pool
      ...
      if (emptyPage == false) {
        try {
          ...
          bst = hashTable.remove(frmeTable[frameNo].pageNo);
          if (bst != true) throw new HashOperationException(
            e,
          ...
          if (bst != true) throw new ReplacerException(
            e,
  ...
  public void unpinPage(PageId PageId_in_a_DB, boolean dirty)
    ...
    if (dirty == true) frmeTable[frameNo].dirty = dirty;
  }
  ...
  public PageId newPage(Page firstpage, int howmany)
    throws BufferPoolExceededException, HashOperationException, ReplacerException, HashEntryNotFoundException, InvalidFrameNumberException, PagePinnedException, PageUnpinnedException, PageNotReadException, BufMgrException, DiskMgrException, IOException {
    int i;
  ...
  public void freePage(PageId globalPageId)
    throws InvalidBufferException, ReplacerException, HashOperationException, InvalidFrameNumberException, PageNotReadException, BufferPoolExceededException, PagePinnedException, PageUnpinnedException, HashEntryNotFoundException, BufMgrException, DiskMgrException, IOException {
    int frameNo;
