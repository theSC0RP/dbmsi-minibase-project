...
class HFDriver extends TestDriver implements GlobalConst {
  ...
  protected boolean test1() {
    ...
    if (status == OK) {
      ...
      if (
        ...
        System.err.println(
          "*** The heap-file scan has not pinned the first page\n"
        );
        status = FAIL;
      }
    }
    ...
    if (status == OK) {
      ...
      while (!done) {
        ...
        if (status == OK && !done) {
          ...
          if (len != reclen) {
            System.err.println(
              "*** Record " + i + " had unexpected length " + len + "\n"
            );
            status = FAIL;
            break;
          } else if (
            ...
            System.err.println(
              "*** The heap-file scan has not left its " + "page pinned\n"
            );
            status = FAIL;
            break;
          }
          ...
          if (
            ...
            System.err.println(
              "*** Record " + i + " differs from what we inserted\n"
            );
            System.err.println(
              "rec.ival: " + rec.ival + " should be " + i + "\n"
            );
            System.err.println(
              "rec.fval: " + rec.fval + " should be " + (i * 2.5) + "\n"
            );
            System.err.println(
              "rec.name: " + rec.name + " should be " + name + "\n"
            );
            status = FAIL;
            break;
          }
      ...
      if (status == OK) {
        if (
          ...
          System.err.println(
            "*** The heap-file scan has not unpinned " +
          ...
          System.err.println(
            "*** Scanned " + i + " records instead of " + choice + "\n"
          );
        }
      }
    }
    System.out.println(
      "\nRead Count: " +
      ReadWriteCounter.getReadCount() +
      "\tWrite Count: " +
      ReadWriteCounter.getWriteCount() +
      "\n"
    );
  ...
  protected boolean test2() {
    ...
    if (
      ...
      System.err.println(
        "t2: in if: getNumbfrs: " + SystemDefs.JavabaseBM.getNumBuffers() + "\n"
      );
    ...
    if (status == OK) System.out.println("  Test 2 completed successfully.\n");
    return status;
  }
  ...
  protected boolean test3() {
    ...
    if (
      ...
      System.err.println(
        "t3, getNumbfrs: " + SystemDefs.JavabaseBM.getNumBuffers() + "\n"
      );
      ...
      while (!done) {
        ...
        if (!done && status == OK) {
          ...
          if (
            ...
            System.err.println(
              "*** Record " + i + " differs from our update\n"
            );
            System.err.println(
              "rec.ival: " + rec.ival + " should be " + i + "\n"
            );
            System.err.println(
              "rec.fval: " + rec.fval + " should be " + (i * 7.0) + "\n"
            );
            status = FAIL;
            break;
          }
    ...
    if (status == OK) System.out.println("  Test 3 completed successfully.\n");
    return status;
  }
  ...
  protected boolean test4() {
    ...
    if (status == OK) System.out.println("  Test 4 completed successfully.\n");
    return (status == OK);
  }
