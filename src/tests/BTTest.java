...
class BTDriver implements GlobalConst {
        ...
        switch (choice) {
          ...
          case 13:
            ...
            if (entry != null) System.out.println(
              "SCAN RESULT: " + entry.key + " " + entry.data
            ); else System.out.println("AT THE END OF SCAN!");
            break;
          case 14:
            scan.delete_current();
            break;
  ...
  void test1(int n) throws Exception {
    ...
    System.out.println("\nRead Count: " + ReadWriteCounter.getReadCount() + "\tWrite Count: " + ReadWriteCounter.getWriteCount() + "\n");
  }
    ...
    try {
      ...
      for (int i = 0; i < m; i++) {
        ...
        if (file.Delete(key, rid) == false) {
          ...
          System.out.println(
            "*     Your delete method has bug!!!                     *"
          );
          System.out.println(
            "*     You insert a record, But you failed to delete it. *"
          );
          System.out.println(
            "*********************************************************"
          );
