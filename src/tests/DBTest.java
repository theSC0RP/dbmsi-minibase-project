...
class DBDriver extends TestDriver implements GlobalConst {
  ...
  protected boolean test1() {
    ...
    if (status == OK) {
      ...
      for (int i = 0; i < 20 && status == OK; ++i) {
        ...
        try {
          ...
          status = FAIL;
          System.err.print(
            "*** Error writing to page " + (runStart.pid + i) + "\n"
          );
          e.printStackTrace();
        }
      }
    ...
    System.out.println("Read Count: " + ReadWriteCounter.getReadCount() + "\tWrite Count: " + ReadWriteCounter.getWriteCount()  + "\n");
    ...
    if (status == OK) {
      ...
      for (int i = 0; i < 20 && status == OK; ++i) {
        ...
        try {
          ...
          status = FAIL;
          System.err.print(
            "*** Error reading from page " + (runStart.pid + i) + "\n"
          );
          e.printStackTrace();
        }
  ...
  protected boolean test4() {
    ...
    if (status == OK) {
      System.out.print(
        "  - Allocate all pages remaining after " +
      ...
      try {
        ...
        e.printStackTrace();
        System.err.print(
          "*** Too little space available: could not " +
          "allocate " +
          (dbsize - 3) +
          " pages\n"
        );
      ...
      if (status == OK) {
        if (pgid.pid != 3) {
          status = FAIL;
          System.err.print(
            "*** Expected the first page allocated to " + "be page 3\n"
          );
        } else if (
          SystemDefs.JavabaseBM.getNumUnpinnedBuffers() !=
          SystemDefs.JavabaseBM.getNumBuffers()
          ...
          status = FAIL;
          System.err.print(
            "*2** The disk space manager has left pages pinned\n"
          );
        } else {
    ...
    if (status == OK) {
      System.out.print(
        "  - Add enough file entries that the directory " +
        "must surpass a page\n"
      );
    ...
    if (status == OK) {
      ...
      if (status == OK) {
        try {
          ...
          status = FAIL;
          System.err.print(
            "*** But allocating the number that " +
    ...
    if (status == OK) {
      System.out.print(
        "  - At this point, all pages should be claimed.  " +
    ...
    if (status == OK) {
      System.out.print(
        "  - Free the last two pages: this tests a boundary " +
