...
class BMDriver extends TestDriver implements GlobalConst {
  ...
  protected boolean test1() {
    ...
    for (
      pid.pid = firstPid.pid, lastPid.pid = pid.pid + numPages;
      ...
      if (status == OK) {
        ...
        if (status == OK) {
          try {
            ...
            status = FAIL;
            System.err.print(
              "*** Could not unpin dirty page " + pid.pid + "\n"
            );
            e.printStackTrace();
          }
        }
      ...
      if (status == OK) {
        ...
        if (status == OK) {
          if (data != (pid.pid) + 99999) {
            status = FAIL;
            System.err.print(
              "*** Read wrong data back from page " + pid.pid + "\n"
            );
          }
        }
  ...
  protected boolean test2() {
    ...
    for (
      pid.pid = firstPid.pid + 1, lastPid.pid = firstPid.pid + numPages - 1;
      status == OK && pid.pid < lastPid.pid;
      pid.pid = pid.pid + 1
    ) {
      try {
  ...
  protected boolean test3() {
    ...
    System.out.print(
      "  - Allocate and dirty some new pages, one at " +
    ...
    for (index = 0; status == OK && index < numPages; ++index) {
      try {
        ...
        status = FAIL;
        System.err.print(
          "*** Could not allocate new page number " + index + 1 + "\n"
        );
        e.printStackTrace();
      }
