...
public class QueryCheck {
  ...
  public QueryCheck(int q_index) {
    ...
    switch (q_index) {
      ...
      case 5:
        ...
        for (int i = 0; i < mygroup[0].len; i++) { // set tuple value.
          try {
            ...
            mygroup[0].mytuple[i].setIntFld(
                2,
                ((S5) Q5result.elementAt(i)).rating
              );
            mygroup[0].mytuple[i].setFloFld(
                3,
      ...
      default:
        System.out.print(
          "Before using querycheck, must set " + "up your answer first.\n\n"
        );
    }
  ...
  public void Check(Tuple t) {
    ...
    if (mygroup[curGroup].order.order == Order.SORT) {
      ...
      try {
        ...
        if (
          tUtil.Equal(
            mygroup[curGroup].mytuple[count],
            t,
            types,
            (int) columnum
          )
      ...
      for (int i = 0; i < mygroup[curGroup].len; i++) {
        try {
          TupleUtils tUtil = new TupleUtils();
          if (
            (mygroup[curGroup].mark[i] == 0) &&
            (
              tUtil.Equal(
                mygroup[curGroup].mytuple[i],
                t,
                types,
                (int) columnum
              )
  ...
  public void report(int querynum) {
    if (total < tuplenum) System.out.print(
    ...
    if (missing != null || extra != null) {
      ...
      System.out.print(
        "  list, your tuples are probably not " + "grouped correctly.\n"
      );
    }

    // check group order error flag
    if (G_O_flag != 0) System.out.print(
